...
from .cache import get_cache_dir, get_or_download_cache
...
mcp = FastMCP("spicedocs", instructions="Search and browse NAIF SPICE documentation")
...
def rebuild_index(archive_dir: Path, conn: sqlite3.Connection) -> None:
...
def index_file(file_path: Path, base_dir: Path, conn: sqlite3.Connection) -> None:
...
@mcp.tool()
async def list_pages(filter_pattern: str | None = None, limit: int = 50) -> str:
    """
...
async def extract_links(path: str, internal_only: bool = True) -> str:
    ...
    try:
        ...
        response = (
            f"Found {len(links)} {'internal ' if internal_only else ''}links in '{path}':\n\n"
        )
...
async def get_archive_stats() -> str:
    ...
    try:
        ...
        other_files = [
            f for f in archive_path.rglob("*") if f.is_file() and not f.name.endswith(".html")
        ]
